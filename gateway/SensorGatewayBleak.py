...
class RuuviTagAccelerometerCommunicationBleak(Event_ts):
        ...
        for i in mac:
            ...
            try:
                async with BleakClient(i) as client:
                    ...
                    await client.start_notify(self.ruuvi_commands['communication_channels']['UART_RX'], partial(self.handle_sensor_commands, client))
                    await client.write_gatt_char(self.ruuvi_commands['communication_channels']['UART_TX'],
                    ...
                    await client.stop_notify(self.ruuvi_commands['communication_channels']['UART_RX'])
    ...
    async def connect_to_mac(self, i, readCommand):
        ...
        try:
            ...
            async with BleakClient(i) as client:
                # Send the command (Wait for Response must be True)
                self.client = client
                await client.start_notify(self.ruuvi_commands['communication_channels']['UART_RX'], self.handle_data)
    ...
    def activate_logging_at_sensor(self, specific_mac=""):
        ...
        """
        self.success = False
        self.work_loop(macs=specific_mac, command=self.ruuvi_commands['ruuvi_commands']['activate_logging_at_sensor'])
        if self.success:
    ...
    def deactivate_logging_at_sensor(self, specific_mac=""):
        ...
        """
        self.success = False
        self.work_loop(macs=specific_mac, command=self.ruuvi_commands['ruuvi_commands']['deactivate_logging_at_sensor'])
        if self.success:
    ...
    def get_acceleration_data(self, specific_mac=""):
        ...
        for i in mac:
            ...
            sensordaten = bytearray()
            self.current_mac=i
            taskobj = self.my_loop.create_task(self.connect_to_mac(i, self.ruuvi_commands['ruuvi_commands']['readAllString']))
    ...
    def set_config_sensor(self, specific_mac="", sampling_rate='FF', sampling_resolution='FF', measuring_range='FF',
        ...
        command_string = self.ruuvi_commands['ruuvi_commands']['substring_set_config_sensor'] + hex_sampling_rate + hex_sampling_resolution + hex_measuring_range + "FFFFFF" + hex_divider + "00"
    ...
    def set_sensor_time(self, specific_mac=""):
        ...
        command_string = self.ruuvi_commands['ruuvi_commands']['substring_set_sensor_time'] + timestamp
