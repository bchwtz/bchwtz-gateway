...
if [ ! -d $DIST_DIR ]; then
    ...
    if [ ! -z "$IS_32" ] ; then
        ...
        sed -i -e 's/\/lib\/libdbus-1\.so/\/lib\/arm-linux-gnueabihf\/libdbus-1.so.3/g' docker-compose.std.yml
        sed -i -e 's/\/lib\/libreadline\.so/\/lib\/arm-linux-gnueabihf\/libreadline.so.7/g' docker-compose.std.yml
    fi
